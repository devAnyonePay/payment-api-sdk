...
use AnyonePay\entity\RegisterRes;
use AnyonePay\entity\VerifyRes;
use Unirest\Request;
...
class CheckoutService
{
    ...
    public function regist($registerReq)
    {
        try {
            ...
            $url = $this->apiServerOrigin . '/payments/v1/registerPayment';
    ...
    public function verify($verifyReq)
    {
        try {
            $headers = array(
                'Content-Type' => 'application/json',
                'Accept' => 'application/json',
                'X-Anyonepay-Client-Id' => $verifyReq->clientId,
                'X-Anyonepay-Client-Secret' => $verifyReq->clientSecret,
            );

            $url = $this->apiServerOrigin . '/payments/v1/retrievePayment';
            ...
            $response = Request::get($url, $headers, array(
                'paymentSeq' => $verifyReq->paymentSeq
            ));
            ...
            if ($response->code == 200) {
                ...
                */
                ...
                $this->apiResponse = new VerifyRes($result);
